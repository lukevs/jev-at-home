...
def test_transformers_does_not_import_optional_mlx(monkeypatch) -> None:
    evaluator = object()
    monkeypatch.setattr(
        "jev_at_home.evaluators.TransformersEvaluator.load",
        lambda name, device: evaluator,
    )
    monkeypatch.setattr(
        "jev_at_home.evaluators.import_module",
...
def test_mlx_reports_missing_extra(monkeypatch) -> None:
    ...
    monkeypatch.setattr("jev_at_home.evaluators.import_module", fail_import)
...
def test_mlx_loads_requested_model(monkeypatch) -> None:
    ...
    monkeypatch.setattr("jev_at_home.evaluators.import_module", lambda name: module)
